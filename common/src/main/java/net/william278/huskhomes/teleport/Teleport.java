...
public class Teleport {
    ...
    protected Teleport(@NotNull OnlineUser executor, @NotNull Teleportable teleporter, @NotNull Target target,
                       ...
                       @Nullable final String queueType, @NotNull HuskHomes plugin) {
        this.plugin = plugin;
    ...
    public void execute() throws TeleportationException {
            ...
            fireEvent((event) -> {
                ...
                if (target instanceof Username) {
                    Message.builder()
                            ...
                            .payload(Payload.withString(((Username) target).name()))
        ...
        if (target instanceof Username) {
            final Optional<OnlineUser> localTarget = ((Username) target).name().equals("@s")
                    ? Optional.of(executor) : ((Username) target).findLocally(plugin);
    ...
    private Optional<OnlineUser> resolveLocalTeleporter() throws TeleportationException {
        if (this.teleporter instanceof Username) {
            return ((Username) this.teleporter).findLocally(plugin);
        }
    ...
    protected boolean queue() {
        ...
        if (this.target == null || !(this.target instanceof Position)) {
            return false;
        }
        final var server = ((Position) this.target).getServer();
        if (this.queueType == null || !(this.teleporter instanceof OnlineUser)) {
            return false;
        }
        final var onlineUser = (OnlineUser) this.teleporter;
        if (onlineUser.getPosition().getServer().equals(server)) {
            return false;
        }
        if (onlineUser.hasPermission("huskhomes.queue.bypass.*") ||
            onlineUser.hasPermission("huskhomes.queue.bypass." + server)) {
            return false;
        }
