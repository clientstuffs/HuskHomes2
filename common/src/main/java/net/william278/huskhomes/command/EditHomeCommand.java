...
public class EditHomeCommand extends SavedPositionCommand<Home> {
    ...
    public void execute(@NotNull CommandUser executor, @NotNull Home home, @NotNull String[] args) {
        ...
        switch (operation.get().toLowerCase()) {
            case "rename":
                setHomeName(executor, home, ownerEditing, args);
                break;
            case "description":
                setHomeDescription(executor, home, ownerEditing, args);
                break;
            case "relocate":
                setHomePosition(executor, home, ownerEditing);
                break;
            case "privacy":
                setHomePrivacy(executor, home, ownerEditing, args);
                break;
        }
    }
    ...
    private void setHomePosition(@NotNull CommandUser executor, @NotNull Home home, boolean ownerEditing) {
        ...
        home.update(((OnlineUser) executor).getPosition());
        plugin.fireEvent(plugin.getHomeEditEvent(home, executor), (event) -> {
            try {
    ...
    private void setHomePrivacy(@NotNull CommandUser executor, @NotNull Home home, boolean ownerEditing, @NotNull String[] args) {
        ...
        if (executor instanceof OnlineUser && !plugin.validateEconomyCheck(((OnlineUser) executor), EconomyHook.Action.MAKE_HOME_PUBLIC)) {
            return;
        }
        ...
        plugin.fireEvent(plugin.getHomeEditEvent(home, executor), (event) -> {
            try {
                ...
                int maxHomes = plugin.getManager().homes().getMaxPublicHomes(executor instanceof OnlineUser ? ((OnlineUser) executor) : null);
            ...
            // Perform transaction
            if (executor instanceof OnlineUser) {
                plugin.performEconomyTransaction(((OnlineUser) executor), EconomyHook.Action.MAKE_HOME_PUBLIC);
            }
