...
public class TpCommand extends Command implements TabProvider {
    ...
    public void execute(@NotNull CommandUser executor, @NotNull String[] args) {
        switch (args.length) {
            case 1:
                ...
                this.execute(executor, ((OnlineUser) executor), Target.username(args[0]), args);
                break;
            case 2:
            ...
            default:
                ...
                if (target.isPresent()) {
                    ...
                    this.execute(executor, ((OnlineUser) executor), target.get(), args);
                    return;
                }
    ...
    private void execute(@NotNull CommandUser executor, @NotNull Teleportable teleportable, @NotNull Target target,
        ...
        try {
            if (executor instanceof OnlineUser) {
                builder.executor(((OnlineUser) executor));
            }
        ...
        final String teleporterName = teleportable instanceof OnlineUser
                ? ((OnlineUser) teleportable).getUsername() : ((Username) teleportable).name();
        if (target instanceof Position) {
            plugin.getLocales().getLocale("teleporting_other_complete_position", teleporterName,
                            Integer.toString((int) ((Position) target).getX()), Integer.toString((int) ((Position) target).getY()),
                            Integer.toString((int) ((Position) target).getZ()))
                    .ifPresent(executor::sendMessage);
        } else {
